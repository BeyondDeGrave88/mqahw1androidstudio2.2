...
//    }
    ...
    @Test
    fun testTextDoNotChangedOnEmpty() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
    ...
    @Test
    fun testTextShowedOnNewActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textToSetNew
        device.findObject(By.res(packageName, "buttonChange")).click()
        Thread.sleep(TIMEOUT)
        ...
        assertEquals(result, textToSetNew)
    }
